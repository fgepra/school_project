...
// ─── 운동 정의 ────────────────────────────────────────────────
const EXERCISES: Record<ExerciseKey, { label: string; icon: string; desc: string }> = {
  free:   { label: '자유 모드',  icon: '🦴', desc: '골격 감지만 표시합니다.' },
  squat:  { label: '스쿼트',    icon: '🏋️', desc: '무릎을 90° 가까이 굽혀보세요.' },
  pushup: { label: '푸시업',    icon: '💪', desc: '팔꿈치가 90°일 때 최저점입니다.' },
  plank:  { label: '플랭크',    icon: '🔥', desc: '어깨·엉덩이·발목을 일직선으로 유지하세요.' },
};
