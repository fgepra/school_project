...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      paymentApi.checkPurchase(courseId).then((res) => setPurchased(res.data.purchased)).catch(() => {});
    }
      ...
      {lectures.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {lectures.map((lecture, index) => {
            ...
            const isPaidAndNotPurchased = (course.price ?? 0) > 0 && !purchased;

            return (
              <Link
                key={lecture.id}
                href={isPaidAndNotPurchased ? `/payment/${id}` : `/courses/${id}/lectures/${lecture.id}`}
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      height: 32,
                      borderRadius: '50%',
                      background: isPaidAndNotPurchased ? 'var(--bg-elevated)' : 'var(--accent)',
                      border: isPaidAndNotPurchased ? '1px solid var(--border)' : 'none',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      flexShrink: 0,
                      fontSize: isPaidAndNotPurchased ? 14 : 12,
                    }}
                  >
