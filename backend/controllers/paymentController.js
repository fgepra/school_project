...
exports.confirmTossPayment = async (req, res) => {
  const userId = req.user.id;
  const { paymentKey, orderId, amount, courseId } = req.body;
  ...
  // 강의 정보 확인
  db.query('SELECT id, title, price FROM courses WHERE id = ?', [courseId], async (err, courses) => {
    if (err || !courses.length) return res.status(404).json({ success: false, message: '강의를 찾을 수 없습니다.' });

    const course = courses[0];
    if (Number(course.price) !== Number(amount)) {
      return res.status(400).json({ success: false, message: '결제 금액이 일치하지 않습니다.' });
    }
    ...
    db.query(
      ...
      async (err, existing) => {
        ...
        const secretKey = process.env.TOSS_SECRET_KEY || 'test_sk_docs_OePez8L93EZmdG9G4Oy8W0RRLBl1lDK';
        ...
        try {
          ...
          db.query(
            'INSERT INTO payments (user_id, course_id, amount, status, payment_method, card_last4) VALUES (?,?,?,?,?,?)',
            [userId, courseId, amount, 'completed', '토스페이', null],
            (err, result) => {
              if (err) return res.status(500).json({ success: false, message: 'DB 오류' });

              createNotification(userId, 'payment', '결제 완료', `${course.title} 강의가 결제되었습니다.`, courseId);
              db.query('SELECT name FROM users WHERE id = ?', [userId], (_, users) => {
                const userName = users?.[0]?.name || '사용자';
                alerts.newPurchase(userName, course.title, amount).catch(() => {});
              });

              res.json({ success: true, message: '결제가 완료되었습니다.', data: { paymentId: result.insertId } });
            }
          );
        } catch (e) {
          res.status(500).json({ success: false, message: '토스 결제 확인 중 오류가 발생했습니다.' });
        }
      }
    );
  });
